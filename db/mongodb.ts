...
class Database {
    ...
    async getLogs(query = {}, collectionName, sort = {}, limit = Infinity, skip = 0): Promise<any[{}]> {
        try {
            const collection = database.collection(collectionName);
    ...
    async addLogs(body, collectionName, multiple = false) {
        const data = body ?? {};
        const collection = database.collection(collectionName);
        if (multiple && !Array.isArray(data)) throw Error("required any array for multiple insertion!")
        const res = !multiple ? await collection.insertOne(data) : await collection.insertMany(data);
        return res;
    }
